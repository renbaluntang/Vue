<template>
  <div class="p-4 sm:p-6 lg:p-10 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
    ...
    <section class="flex flex-col lg:flex-row gap-6 justify-between items-start lg:items-center">
      <div class="space-y-1">
        <div class="flex items-center gap-2">
          <span class="px-2.5 py-0.5 rounded-full bg-emerald-100 text-emerald-800 text-[11px] font-extrabold uppercase tracking-wider flex items-center gap-1">
            <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> Online English Portal
          </span>
          <span class="text-xs text-slate-400 font-semibold">• {{ user.profile.level }}</span>
        </div>
        <h1 class="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-slate-900 tracking-tight">
          Welcome back, <span class="text-indigo-600">{{ user.profile.firstName }}</span> 👋
        </h1>
      ...
      <div class="flex flex-wrap gap-2.5 sm:gap-3 w-full lg:w-auto">
        <RouterLink
          to="/booking"
          class="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-5 py-3.5 rounded-2xl bg-[#FFCD00] text-black font-bold text-sm shadow-md hover:bg-[#FFD933] hover:shadow-lg hover:scale-105 active:scale-95 transition-all"
        >
          <span>🗓️</span> Book Online Class
        </RouterLink>

        <button
          @click="showFreeConversationModal = true"
          class="relative group flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-5 py-3.5 rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold text-sm shadow-lg shadow-blue-500/20 hover:scale-105 active:scale-95 transition-all"
        >
          <span class="absolute -top-2 -right-2 bg-indigo-600 text-white text-[9px] uppercase tracking-wider font-black px-2 py-0.5 rounded-full shadow-md animate-pulse">
            Instant
          </span>
          <span>⚡</span> Free Conversation
        </button>
    ...
    <section class="space-y-4">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div class="flex items-center gap-2.5">
          <span class="w-2.5 h-6 rounded-full bg-indigo-600"></span>
        ...
        <div class="flex items-center gap-3">
          <RouterLink to="/booking" class="text-xs font-bold text-indigo-600 hover:text-indigo-600">
            + Book Another Class
          </RouterLink>
      ...
      <div class="bg-white border border-slate-200/80 rounded-3xl shadow-sm overflow-hidden">
        ...
        <div
          ...
          class="relative overflow-hidden text-white p-6 sm:p-8 border-b border-slate-900/50 bg-[radial-gradient(70%_90%_at_88%_6%,rgba(99,102,241,0.20),transparent_60%),radial-gradient(55%_80%_at_4%_100%,rgba(79,70,229,0.12),transparent_65%),linear-gradient(135deg,#0E1220_0%,#1B2140_52%,#0A0D1A_100%)]"
        >
            ...
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
              ...
              <div class="flex items-start gap-4 sm:gap-5 max-w-2xl">
                ...
                >
                  <img
                    ...
                    class="w-20 h-20 sm:w-24 sm:h-24 mx-auto rounded-full object-cover border-2 border-indigo-400/70 transition group-hover:border-indigo-300 group-hover:scale-105"
                  />
                  <span class="mt-2 block truncate text-xs font-bold text-white transition group-hover:text-indigo-600">
                    {{ user.nextUpcomingClass.teacherName }}
                  </span>
                  <span class="block text-[11px] font-semibold text-indigo-600 transition group-hover:text-indigo-600">
                    View profile →
                  </span>
                </button>
                ...
                <div class="min-w-0 pt-1">
                  ...
                  <p class="text-indigo-600 text-sm font-medium mt-1.5">
              ...
              <div class="flex flex-col sm:flex-row gap-3 w-full lg:w-auto shrink-0">
                <a
                  ...
                  target="_blank"
                  class="inline-flex items-center justify-center gap-2 px-6 py-4 rounded-2xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-base shadow-lg shadow-emerald-500/20 hover:scale-105 active:scale-95 transition-all text-center whitespace-nowrap"
                >
                ...
                <button
                  ...
                  class="inline-flex items-center justify-center px-6 py-4 rounded-2xl bg-white/10 hover:bg-white/20 text-white font-bold text-sm transition border border-white/15 text-center whitespace-nowrap"
                >
                  Class Details
                </button>
              </div>
        ...
        <div class="sched p-4 sm:p-6 bg-white space-y-4">
          <div class="flex items-center justify-between gap-2 pb-1 border-b border-slate-100/80">
            <div class="flex items-center gap-2">
              <span class="text-xs font-bold uppercase tracking-wider text-slate-400">Upcoming Lesson Schedule</span>
              <span class="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-extrabold text-slate-600 border border-slate-200">
                {{ scheduledLessons.length }} after this one
              </span>
            </div>
            <RouterLink
              to="/booking"
              class="text-[11px] font-bold text-indigo-600 hover:underline inline-flex items-center gap-1 shrink-0"
            >
            ...
            <table class="w-full text-left border-collapse min-w-[650px]">
              ...
              <tbody class="divide-y divide-slate-100 text-sm">
                <tr
                  ...
                  :class="lesson.isNext ? 'bg-indigo-50/50' : ''"
                >
                  ...
                  <td class="py-3.5 px-4">
                    ...
                    >
                      <img
                        ...
                        class="w-10 h-10 rounded-full object-cover ring-2 ring-white outline outline-1 outline-slate-200 shadow-2xs transition group-hover/teacher:outline-indigo-300"
                      />
                      <span class="min-w-0">
                        <span class="block truncate font-bold text-slate-800 transition group-hover/teacher:text-indigo-600">
                          {{ lesson.teacherName }}
                        </span>
                        <span
                  ...
                  <td class="py-3.5 px-4 text-right whitespace-nowrap">
                    <div class="inline-flex items-center justify-end gap-2 flex-nowrap">
                      <a
                        ...
                        target="_blank"
                        class="inline-flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-xl bg-blue-50 text-blue-600 font-bold text-xs hover:bg-blue-100 hover:scale-105 active:scale-95 transition whitespace-nowrap flex-shrink-0 shadow-2xs cursor-pointer"
                      >
                        <span>📹</span>
                        <span>Join Meet</span>
                      </a>
                      <button
                        type="button"
                        @click="rescheduleTargetLesson = lesson"
                        class="inline-flex items-center justify-center gap-1 px-3 py-2 rounded-xl border border-amber-200 bg-amber-50/70 text-amber-900 font-bold text-xs hover:bg-amber-100/80 active:scale-95 transition whitespace-nowrap flex-shrink-0 cursor-pointer"
                      ...
                      <button
                        ...
                        class="inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl border border-slate-200 bg-white text-slate-700 font-bold text-xs hover:border-slate-300 hover:bg-slate-50 active:scale-95 transition whitespace-nowrap flex-shrink-0 cursor-pointer"
          ...
          <!-- Narrow / Mobile View: Restructured 2-line Thumb-Sized Stacked List -->
          <div class="sched__rows border border-slate-200/90 rounded-2xl overflow-hidden bg-white divide-y divide-slate-100 shadow-2xs">
            <div
              ...
              :class="[
                lesson.isNext ? 'bg-indigo-50/30' : 'hover:bg-slate-50/70',
              ...
              >
                ...
                <div class="flex items-center justify-between gap-2 mt-1">
                  ...
                  <div class="flex items-center gap-1.5 flex-wrap min-w-0">
                    ...
                    <span
                      v-if="lesson.isNext"
                      class="inline-flex items-center rounded bg-indigo-50 px-1.5 py-0.5 text-[9px] font-extrabold text-indigo-700 border border-indigo-200 shrink-0"
                    >
                      Next up
                    </span>
                  ...
                  <div class="flex items-center gap-1 text-[11px] font-bold text-slate-400 group-hover:text-slate-600 shrink-0">
                    <i
                      ...
                      :class="{ 'rotate-180 text-indigo-600': isLessonOpen(lesson) }"
                    ></i>
                  </div>
                </div>
              </div>

              <!-- Expanded In-Place Drawer -->
              <div
              ...
              >
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                  ...
                  <div class="flex items-center justify-between p-2.5 rounded-xl bg-white border border-slate-200/80">
                    ...
                    <button
                      ...
                      class="ml-2 text-[11px] font-bold text-indigo-600 hover:underline shrink-0 cursor-pointer"
                    >
                      Teacher Profile →
                    </button>
                  </div>
                ...
                <div class="pt-1 flex items-center justify-between gap-2 flex-wrap">
                  <div class="flex items-center gap-1.5 flex-1 min-w-0">
                    ...
                    <button
                      type="button"
                      @click.stop="rescheduleTargetLesson = lesson"
                  ...
                  <div class="flex items-center gap-1.5 shrink-0">
                    <a
                      ...
                      target="_blank"
                      @click.stop
                      class="inline-flex items-center justify-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs shadow-xs active:scale-95 transition cursor-pointer"
                    >
                      <span>📹</span> Join Meet
                    </a>
                    <button
          ...
          <p v-if="!scheduledLessons.length" class="py-6 text-center text-sm font-medium text-slate-400">
            ...
            <RouterLink to="/booking" class="font-bold text-indigo-600 hover:underline">
              Book another class →
            </RouterLink>
          </p>
        </div>
        ...
        <div class="bg-slate-50 p-4 border-t border-slate-100 text-[11px] text-slate-500 font-medium flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <RouterLink to="/booking" class="font-bold text-indigo-600 hover:text-indigo-600 whitespace-nowrap">
    ...
    <section class="bg-white border border-slate-200/80 rounded-3xl p-6 sm:p-8 shadow-xs space-y-5">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div
          ...
          :key="instructor.id"
          class="border border-slate-200/80 rounded-2xl p-4 hover:border-indigo-300 hover:shadow-md transition-all flex flex-col justify-between bg-slate-50/50 group"
        >
          <div class="flex items-start gap-3">
            ...
            >
              ...
              />
              <span class="absolute -bottom-1 -right-1 bg-indigo-600 text-white rounded-full p-0.5 text-[8px]">
                🔍
              </span>
            </div>
            ...
            <div class="min-w-0 flex-1">
              <div class="flex items-center justify-between">
                <h3
                  @click="openTeacherDetails(instructor)"
                  class="text-sm font-bold text-slate-900 truncate hover:text-indigo-600 cursor-pointer"
                >
                  {{ instructor.name }}
                </h3>
    ...
    <section class="space-y-4">
      <div class="flex items-center justify-between">
        <h2 class="text-lg sm:text-xl font-extrabold text-slate-900 flex items-center gap-2">
          <span class="w-2 h-5 rounded-full bg-indigo-600"></span> In-Progress Writing Corrections
        </h2>
        <RouterLink to="/writing" class="text-xs font-bold text-indigo-600 hover:text-indigo-600">
          Open Writing Chat →
        </RouterLink>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div
          ...
          :key="task.id"
          class="bg-white border border-slate-200/80 rounded-3xl p-5 hover:border-indigo-300 hover:shadow-sm transition-all group cursor-pointer flex flex-col justify-between"
        ...
        >
          ...
          <div class="flex items-center justify-between mt-4 pt-3 border-t border-slate-100">
            ...
            <span class="text-xs font-bold text-indigo-600 group-hover:translate-x-1 transition-transform">Reply Chat →</span>
          </div>
        </div>
      </div>
    </section>
