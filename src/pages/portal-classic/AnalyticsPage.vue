<template>
  <div class="p-4 sm:p-6 lg:p-10 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 border border-indigo-200 text-indigo-600 text-xs font-bold mb-2">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3.5 sm:gap-5">
      ...
      <div class="bg-white border border-slate-200/80 rounded-3xl p-5 shadow-xs flex flex-col justify-between hover:shadow-md transition">
        ...
        <div>
          ...
          <div class="text-[11px] font-semibold text-indigo-600 mt-1">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8">
      ...
      <div class="lg:col-span-7 bg-white border border-slate-200/80 rounded-3xl p-6 sm:p-8 shadow-xs space-y-6">
        <div class="flex items-center justify-between border-b border-slate-100 pb-4">
          <div>
            <h2 class="text-lg font-bold text-slate-900 flex items-center gap-2">
              <span class="w-2 h-5 rounded-full bg-indigo-600"></span> English Skills Assessment
            </h2>
        ...
        <div class="rounded-2xl bg-indigo-50/70 border border-indigo-100 p-4 text-xs text-white flex items-start gap-3">
      ...
      <div class="lg:col-span-5 bg-white border border-slate-200/80 rounded-3xl p-6 sm:p-8 shadow-xs space-y-6 flex flex-col justify-between">
        <div>
          ...
          <div class="grid grid-cols-7 gap-1.5 mt-6 text-center">
            ...
            >
              <div class="text-[9px] uppercase">{{ day }}</div>
              <div class="mt-1 text-sm">{{ idx < 4 ? '✓' : idx === 4 ? 'Today' : '—' }}</div>
            </div>
          </div>
        </div>
    ...
    <div class="bg-white border border-slate-200/80 rounded-3xl p-6 sm:p-8 shadow-xs space-y-6">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
        ...
        <div class="text-xs font-bold text-slate-500">
          Unlocked: <strong class="text-indigo-600 font-extrabold">{{ user.badges.filter(b => b.unlocked).length }}</strong> of {{ user.badges.length }}
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
        <div
          ...
          :class="badge.unlocked ? 'border-slate-200 bg-white hover:border-indigo-300 hover:shadow-md' : 'border-dashed border-slate-200 bg-slate-50/70 opacity-75'"
        >
