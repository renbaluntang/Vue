  ...
  <div class="p-4 sm:p-6 lg:p-10 max-w-5xl mx-auto space-y-6 animate-in fade-in duration-500">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
      <div class="flex items-center gap-3">
        <span class="flex h-11 w-11 items-center justify-center rounded-2xl bg-indigo-50 text-xl">📜</span>
        <div>
        ...
        <div class="flex shrink-0 flex-wrap items-center gap-2 self-end sm:self-center">
          ...
          >
            <span v-if="lesson.yourRating" class="tracking-[0.15em] text-sm leading-none">
              <span class="text-indigo-700">{{ '★'.repeat(lesson.yourRating) }}</span><span class="text-indigo-300">{{ '★'.repeat(5 - lesson.yourRating) }}</span>
            </span>
    ...
    >
      ...
      >
        <div class="flex items-start justify-between gap-4 border-b border-slate-100 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 p-5 text-white sm:p-6">
          <div class="min-w-0">
            <p class="text-[11px] font-bold uppercase tracking-wider text-indigo-600">
        ...
        <div class="p-6 text-center">
          ...
          <div class="mt-5 flex items-center justify-center gap-1.5" role="radiogroup" aria-label="Rating out of five">
            <button
              ...
              class="rounded-lg p-1 text-3xl leading-none transition hover:scale-110 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-600"
              :class="star <= draftStars ? 'text-indigo-700' : 'text-slate-200 hover:text-indigo-400'"
            >
              ★
            </button>
          </div>
          ...
          <textarea
            ...
            class="mt-4 w-full resize-none rounded-2xl border border-slate-200 bg-white px-3.5 py-3 text-sm text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-indigo-400 focus:ring-4 focus:ring-indigo-200"
          ></textarea>
        </div>
