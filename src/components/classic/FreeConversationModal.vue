<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm transition-opacity duration-300">
    <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden transform scale-100 transition-transform duration-300 animate-in zoom-in-95">
      <!-- Header -->
      <div class="bg-gradient-to-br from-indigo-600 to-blue-600 p-6 text-white text-center relative">
        ...
        <p class="text-blue-100 text-sm mt-1">Start a 25-minute lesson right now.</p>
      </div>
      ...
      <div v-else class="p-8 text-center space-y-4">
        ...
        <div class="bg-blue-50 border border-blue-100 rounded-xl p-4 mt-4">
          <p class="text-xs text-blue-500 font-bold uppercase tracking-wider mb-1">Assigned Teacher</p>
        ...
        <button 
          @click="$emit('close')"
          class="w-full mt-6 py-3 rounded-2xl bg-blue-600 text-white font-bold shadow-md transition-all hover:bg-blue-700"
        >
          Join Google Meet
        </button>
      </div>
