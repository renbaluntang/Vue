  ...
  >
    ...
    >
      ...
      <div class="shrink-0 flex items-start justify-between gap-4 border-b border-slate-100 bg-gradient-to-r from-slate-900 via-slate-950 to-slate-900 p-4 text-white sm:p-6">
        <div class="min-w-0">
          <p class="text-[11px] font-bold uppercase tracking-wider text-brighture-gold">Edit class</p>
          <h2 id="edit-class-title" class="mt-1 truncate text-lg font-extrabold sm:text-xl">
            {{ lesson.teacherName }}
          </h2>
          <p class="mt-0.5 text-xs font-semibold text-brighture-gold">{{ lesson.date }} · {{ lesson.time }}</p>
        </div>
        <button
          @click="$emit('close')"
      ...
      <div class="flex-1 overflow-y-auto space-y-4 p-4 sm:space-y-5 sm:p-6">
        ...
        <div>
          ...
          <select
            id="edit-subject"
            v-model="subject"
            class="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3.5 py-3 text-sm font-semibold text-slate-900 transition focus:border-brighture-amber focus:outline-none focus:ring-4 focus:ring-brighture-gold/10"
          >
        ...
        <div>
          ...
          <textarea
            ...
            class="mt-1.5 w-full resize-none rounded-xl border border-slate-200 bg-white px-3.5 py-3 text-sm text-slate-900 transition placeholder:text-slate-400 focus:border-brighture-amber focus:outline-none focus:ring-4 focus:ring-brighture-gold/10"
          ></textarea>
      ...
      <!-- Actions -->
      <div class="shrink-0 flex items-center justify-end gap-2 border-t border-slate-100 bg-slate-50 p-3 sm:gap-2.5 sm:p-4">
        <button
          @click="$emit('close')"
        ...
        <button
          ...
          :class="`rounded-xl px-3 py-2.5 text-[11px] font-bold shadow-xs transition sm:px-5 sm:text-xs ${
            isDirty
